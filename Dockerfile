...
RUN git clone --depth 1 https://github.com/rehosting/dwarf2json.git \
    && cd dwarf2json \
    && go build
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/i686-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
FROM base AS x86_64
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/x86_64-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
FROM base AS armel
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/arm-linux-musleabi-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
FROM base AS arm64
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/aarch64-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
FROM base AS mipseb
RUN mkdir -p /opt/cross && \
    wget http://panda.re/secret/mipseb-linux-musl_gcc-5.3.0.tar.gz -O - | tar --no-same-owner -xz -C /opt/cross && \
    bash /opt/cross/setup-cross.sh
...
FROM base AS mipsel
RUN mkdir -p /opt/cross && \
    wget http://panda.re/secret/mipsel-linux-musl_gcc-5.3.0.tar.gz -O - | tar --no-same-owner -xz -C /opt/cross && \
    bash /opt/cross/setup-cross.sh
...
RUN mkdir -p /opt/cross && \
    wget http://panda.re/secret/mips64-linux-musl-cross_gcc-6.5.0.tar.gz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/mips64el-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
FROM base AS armhf
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/arm-linux-musleabihf-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/riscv32-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN apt-get update && apt-get install -y gcc-riscv64-linux-gnu && rm -rf /var/lib/apt/lists/*
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/riscv64-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/powerpc64-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/powerpc64le-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/powerpc-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://musl.cc/powerpcle-linux-musl-cross.tgz -O - | tar --no-same-owner -xz -C /opt/cross && \
...
RUN mkdir -p /opt/cross && \
    wget https://github.com/loongson/build-tools/releases/download/2025.02.21/x86_64-cross-tools-loongarch64-binutils_2.44-gcc_14.2.0-glibc_2.41.tar.xz -O - | tar --no-same-owner -xJ -C /tmp && \
